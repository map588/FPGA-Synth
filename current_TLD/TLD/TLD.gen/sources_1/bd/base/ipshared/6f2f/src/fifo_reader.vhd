...
architecture Behavioral of fifo_reader is
  ...
  signal current_index : integer range 0 to NUM_VOICES := 0;
...
begin
  ...
  begin
    if rst = '1' then
      ...
      if valid = '1' then
        data <= data_in;
        data1 <= data(DATA_WIDTH - 3 downto DATA_WIDTH - 9);
        data2 <= data(DATA_WIDTH - 10 downto DATA_WIDTH - 16);
        if message_type(1) = '0' then
          note_number <= data1;
          velocity <= data2;
          trig := '1';
          note_state <= message_type(0);
      ...
      if trig_signal = '1' then
        if note_state = '1' then -- Note On
          ...
          for i in 0 to NUM_VOICES - 1 loop
            if note_states(i) = note_number then
