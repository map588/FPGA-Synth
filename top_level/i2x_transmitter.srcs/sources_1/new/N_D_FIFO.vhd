...
begin
  ...
  count_gen: process (wr_clk, rd_clk, reset_sync_wr, reset_sync_rd)
  begin
  ...
  full_gen: process (wr_clk, reset_sync_wr)
    variable msbs_not_equal  : boolean := false; -- Local variable
    variable otherwise_equal : boolean := false; -- Local variable
  begin
    if reset_sync_wr = '1' then
      full_int <= '0';
    elsif rising_edge(wr_clk) then
      msbs_not_equal := wr_gray(ADDR_WIDTH downto ADDR_WIDTH - 1) /= rd_gray_sync(ADDR_WIDTH downto ADDR_WIDTH - 1);
      otherwise_equal := wr_gray(ADDR_WIDTH - 2 downto 0) = (rd_gray_sync(ADDR_WIDTH - 2 downto 0));
      if (msbs_not_equal and otherwise_equal) then
        full_int <= '1';
      else
        full_int <= '0';
      end if;
